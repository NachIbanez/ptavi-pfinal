...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as my_socket:
    ...
    if str.upper(method) == "REGISTER":
        ...
        lista = (message.split())
        if lista[1] == "401":
            log(Log_Path, "error", Proxy_IP, Proxy_Port,
                message[:message.find("WWW")-2])
            LINE = (str.upper(method) + " sip:" + UA_Name + ":" +
                    Server_Port +
                    " SIP/2.0\r\n\r\n" + "Expires: " + option + "\r\n" +
                    "Authorizathion: Digest response= " + UA_Password)
            LINE_log = (str.upper(method) + " sip:" + UA_Name + ":"
                        + Server_Port +
                        " SIP/2.0 " + "Expires: " + option +
                        " Authorizathion: Digest response= " + UA_Password)
            log(Log_Path, "send", Proxy_IP, Proxy_Port, LINE_log)
    ...
    if str.upper(method) == "INVITE":
        ...
        data = my_socket.recv(1024)
        message = data.decode('utf-8')
        log(Log_Path, "receive", Proxy_IP, Proxy_Port,
            " ".join(message.split()))
        lista = (message.split())
